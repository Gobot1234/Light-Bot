...
class Staff(Cog):
    ...
    @group(invoke_without_subcommand=True, aliases=["prefixes"])
    @commands.guild_only()
    ...
    async def prefix(self, ctx: Context):
        ...
        if ctx.invoked_subcommand is None:
            embed = discord.Embed(
                title="Your current prefixes are",
                description=human_join(self.bot.configs[ctx.guild.id].prefixes + self.bot.user.mention),
    ...
    async def prefix_add(self, ctx: Context, prefix: str):
        ...
        prefixes = self.bot.configs[ctx.guild.id].prefixes
        if len(prefixes) >= 10:
        ...
        await Config.insert(prefixes=prefixes, guild_id=ctx.guild.id, update_on_conflict=Config.prefixes)
