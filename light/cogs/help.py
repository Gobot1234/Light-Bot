...
class HelpCommand(commands.HelpCommand):
    ...
    def get_command_signature(self, command) -> str:
        ...
        if not command.parent:  # checking if it has args and isn't a subcommand
    ...
    async def send_bot_help(self, mapping):
        ...
        cogs = [
            ...
            if await discord.utils.maybe_coroutine(obj.cog_check, ctx) and name != "owner"
        ]
        cogs.sort()
        ...
        help_embed = await ctx.send(embed=embed)  # sends the first_ready help page
        ...
        while 1:
            try:
                ...
                if str(reaction.emoji) == "⏭":  # go to the last the page
                ...
                elif str(reaction.emoji) == "⏮":  # go to the first_ready page
                    page = 0
                    embed = await self.bot_help_paginator(page, cogs)
                    ...
                    page += 1
                    if page == len(cogs):  # check whether to go to the first_ready page
                        page = 0
                    embed = await self.bot_help_paginator(page, cogs)
                    await help_embed.edit(embed=embed)
                    ...
                    embed = discord.Embed(
                        title=f"Help with {bot.user.name}'s commands",
                        description=bot.description,
                        color=discord.Colour.blurple(),
                    )
                    embed.add_field(
                        name=(
                            f'Currently there are {len(cogs)} cogs loaded, which includes (`{"`, `".join(cogs)}`)'
                            " :gear:"
                        ),
                        value=(
                    ...
                    embed.set_author(
                        name=f"You were on page {page + 1}/{len(cogs)} before", icon_url=ctx.author.avatar_url
                    )
                    embed.set_footer(
    ...
    async def bot_help_paginator(self, page: int, cogs) -> discord.Embed:
        ...
        bot = ctx.bot
        all_commands = [
            command for command in await self.filter_commands(bot.commands)
        ]  # filter the commands the user can use
        cog = bot.get_cog(cogs[page])  # get the current cog

        embed = discord.Embed(
            title=f"Help with {cog.qualified_name} ({len(all_commands)} commands)",
    ...
    async def send_cog_help(self, cog):
        ...
        embed = discord.Embed(
            title=f"Help with {cog.qualified_name} ({len(cog_commands)} commands)",
            description=cog.description,
            color=get_colour(ctx),
        )
        embed.set_author(
            name=f"We are currently looking at the module {cog.qualified_name} and its commands",
    ...
    async def send_group_help(self, group):
        ...
        embed = discord.Embed(
            title=f"Help with `{group.name}`", description=bot.get_command(group.name).help, color=get_colour(ctx)
        )
        embed.set_author(
    ...
    async def command_not_found(self, string):
        ...
        embed = discord.Embed(
            title="Error!",
            description=(
                f'**Error 404:** Command or category "{string}" not found ¯\_(ツ)_/¯\nPerhaps you meant:\n{joined}'
            ),
            color=ctx.bot.config_cache[ctx.guild.id]["colour_bad"] if ctx.guild else discord.Colour.red(),
        )
        embed.add_field(
            name="The current loaded cogs are", value=f'(`{"`, `".join([cog for cog in ctx.bot.cogs])}`) :gear:'
        )
...
class Help(commands.Cog):
    ...
    async def ping(self, ctx):
        ...
        embed.add_field(name=f"{ctx.emoji.discord} Discord latency is:", value=f"`{discord_duration:.2f}` ms.")
        embed.add_field(name=f"{ctx.emoji.text} Message latency is:", value=f"`{message_duration:.2f}` ms.")
    ...
    async def stats(self, ctx):
        ...
        embed.add_field(
            name="Members",
            value=(
                f"`{total_members}` {ctx.emoji.discord} total\n"
        ...
        embed.add_field(
            name="Statuses",
            value=(
                f"`{total_online}` {ctx.emoji.discord} online,\n"
        ...
        embed.add_field(
            ...
            value=(
                f"Using `{naturalsize(rawram[3])}` / `{naturalsize(rawram[0])}`"
                f" `{round(rawram[3] / rawram[0] * 100, 2)}`% "
            ),
        )
        ...
        embed.add_field(
            ...
            value=f"Your command prefix is `{ctx.prefix}`. Type {ctx.prefix}help to list the commands you can use",
        )
        embed.add_field(
            name="Version info:",
            value=(  # TODO add more version info
                f"{ctx.emoji.dpy}: `{discord.__version__}`, "
                f"{ctx.emoji.postgres}: `{asyncpg.__version__}`"
                f"{ctx.emoji.python}: `{python_version()}`"
            ),
            inline=False,
        )
        embed.set_footer(text="If you need any help join the help server discord.gg", icon_url=ctx.author.avatar_url)
        await ctx.send(embed=embed)
    ...
    async def user(self, ctx, user: discord.Member = None):
        ...
        embed.add_field(
            name=f"{user.display_name} created their account:", value=human_timedelta(user.created_at, accuracy=2)
        )
        embed.add_field(
            name=f"{user.display_name} joined this guild:", value=human_timedelta(user.joined_at, accuracy=2)
        )
        embed.add_field(
            name="Shared guilds", value=human_join(shared_guilds, final="and")
        ...
        if user.premium_since:
            embed.add_field(
                name=f"{user.display_name} has been boosting since:", value=human_timedelta(user.premium_since)
            )
    ...
    async def server(self, ctx, *, server: GuildConverter = None):
        ...
        channel_info = []
        key_to_emoji = {discord.TextChannel: ctx.emoji.text, discord.VoiceChannel: ctx.emoji.voice}
        ...
        fmt = (
            f'{ctx.emoji.online} {member_by_status["online"]}\n'
        ...
        embed.add_field(
            ...
            if len(guild.roles) < 10 and guild == ctx.guild
            else f"{len(guild.roles) - 1} roles",
        )
        await ctx.send(embed=embed)
    ...
    async def steamstats(self, ctx):
        async with ctx.typing():
            ...
            if r.status == 200:
                ...
                cities = {
                    code_to_city.get(service[0]): service[2]
                    for service in data["services"]
                    if code_to_city.get(service[0])
                }
                games = {
                    code_to_game.get(service[0]): service[2]
                    for service in data["services"]
                ...
                gamers = "\n".join(
                    f"{gamer[0]} - {gamer[1]}"
                    for gamer in sorted(
                        gamers.items(), key=lambda kv: (kv[0], kv[1])
                    )
                )
