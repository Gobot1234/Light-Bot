...
class Staff(commands.Cog):
    ...
    **Manage Roles permission is needed for:**  - mute, unmute"""
    ...
    async def do_removal(self, ctx, limit, predicate):
        ...
        if deleted:
            ...
            embed = discord.Embed(
                ...
                description=(
                    f"**The clear command was used in {ctx.channel.mention} by {ctx.author.mention}** "
    ...
    async def mute(
        self, ctx, members: commands.Greedy[discord.Member], *, until: UserFriendlyTime(commands.clean_content)
    ):
        """
        ...
        if muted is None:
            muted = await ctx.guild.create_role(
                name="Muted", colour=0x2F3136, reason="Created automatically as no muted role was found"
            )
    ...
    async def mutelist(self, ctx):
        ...
        paginator = buttons.Paginator(
            title=f'{len(muted_list)} Muted member{"s" if len(muted_list) > 1 else ""}',
            ...
            if isinstance(user, discord.User):  # check if the user is still cached or in another server
                try:
                    ...
                    raise commands.UserInputError(
                        f"User {user} not found? Double check your ID or name again or perhaps they aren't banned"
                    )
    ...
    async def p_add(self, ctx, prefix: str):
        ...
        if prefix.startswith(f"<@{ctx.me.id}>") or prefix.startswith(f"<@!{ctx.me.id}>"):
            ...
            await Config.update_where(prefixes, guild_id=ctx.guild.id)
    ...
    async def logged_events(self, ctx):
        if ctx.invoked_subcommand is None:
            ...
            embed = discord.Embed(
                title=f"Logged Events for {ctx.guild} are",
                description=events or "No events are currently logged",
                colour=self.bot.config_cache[ctx.guild.id]["colour"],
            )
